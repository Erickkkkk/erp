...
<body>
	...
	<div style="width:960;margin:0 auto">
		<div class="cell auto-size padding20 bg-white" id="cell-content">
			<h1 class="text-light">
				报销审核 <span class="mif-hammer place-right"></span>
			</h1>

			<hr class="thin bg-grayLighter">
			<button class="button warning"
				onclick="javascript:window.location.href = basePath + 'travel/queryAllTravelReimburse.do';">
	...
	//时间戳格式化
			...
			$("tbody tr").click(function() {
				...
				$.ajax({
					...
					success:function(data){
						...
						var div=$("<div style=\"padding:30 20 10 20\"></div>").appendTo($("#myreimbursedialog"));
						...
						var div4=$("<div style=\"text-align:right;margin-top:10px\"><span style=\"font-size:12px;color:#0072c6\">【"+data[0].state+"】</span><button style=\"margin-left:10px\" type=\"button\" id=\"recheck\" class=\"button warning\">重审</button><button style=\"margin-left:10\" type=\"button\" id=\"check\" class=\"button success\">审核</button><button style=\"margin-left:10\" type=\"button\" id=\"give\" class=\"button primary\">发放</button></div>").appendTo(div);
						...
						$("#give").click(function(e){
							...
							$.ajax({
								type:'post',
								url:basePath+'travel/updateTravelReimburseState.do',
								data:{'id':id,'state':"已发放"},
								dataType:'text',
								success:function(data){
								if(data==='true'){
									div.remove();
									dialog.close();
									$("#"+id).children('td').eq(5).html("已发放");
									$("#"+id).children('td').eq(5).css("color","#2086bf");
								}else{alert("操作失败！ ")}}
							});
						});
						$("#check").click(function(e){
							...
							$.ajax({
								type:'post',
								url:basePath+'travel/updateTravelReimburseState.do',
								data:{'id':id,'state':"已审核"},
								dataType:'text',
								success:function(data){
								if(data==='true'){
									div.remove();
									dialog.close();
									$("#"+id).children('td').eq(5).html("已审核");
									$("#"+id).children('td').eq(5).css("color","#60a917");
								}else{alert("操作失败！ ")}}
							});
						});
						$("#recheck").click(function(e){
							...
							$.ajax({
								type:'post',
								url:basePath+'travel/updateTravelReimburseState.do',
								data:{'id':id,'state':"待审核"},
								dataType:'text',
								success:function(data){
								if(data==='true'){
									div.remove();
									dialog.close();
									$("#"+id).children('td').eq(5).html("待审核");
									$("#"+id).children('td').eq(5).css("color","#fa6800");
								}else{alert("操作失败！ ")}}
							});
						});
					}
		...
		function delreimburse(id){
			...
			window.location.href=basePath+"travel/delTravelReimburse.do?id="+id;
		}
